...
public class GraphUtil {
	 ...
	 * @throws IOException 
	 */
	public static List<Path> readInfeasibleSubpaths(String infeasibleSubpaths) throws InvalidInputException, IOException{
		...
		if(infeasibleSubpaths != null && !infeasibleSubpaths.equals("")){
			...
			while((nextSubPath = br.readLine()) != null){
				...
				if(!Pattern.matches(infeasibleSubpathsPat, nextSubPath.trim()))
					throw new InvalidInputException("An invalid input '" + infeasibleSubpaths + "' for the infeasible subpaths. Please read the notes above the forms. The software would be more error-tolerant if it ignored a trailing \";\". ");
				Path path1 = null;
	 ...
	 * @throws InvalidInputException 
	 */
	public static List<Path> readInfeasibleSubpaths1(String infeasibleSubpaths) throws InvalidInputException{
		//a list of infeasible sub paths
		List<Path> subpaths = new ArrayList<Path>();
		...
		if(infeasibleSubpaths != null && !infeasibleSubpaths.equals("")){
			if(!Pattern.matches(infeasibleSubpathsPat, infeasibleSubpaths.trim()))
				throw new InvalidInputException("An invalid input '" + infeasibleSubpaths + "' for the infeasible subpaths. Please read the notes above the forms. ");
	 ...
	 */
	public static void readDefUse(String d, String u, DFGraph dfg) throws InvalidInputException
	{				
		try
		{
			...
			{	
				...
				if(!Pattern.matches(defusePat, str.trim()))
					throw new InvalidInputException("Could not understand the format of the defs for '" + str+ "'. " +
					...
					else
					{
						...
						if(defsString.indexOf(",") != -1){
							StringTokenizer defsST = new StringTokenizer(defsString, ",");
								...
								Edge e = dfg.findEdge(srcString, destString);
								if(e == null)
									throw new InvalidInputException("Could not find the edges have defs. Please check the defs in the uses box");
								if(e != null)
									var.addDefOnEdges(e);
						}	
						else{
							...
							if(n == null)
								throw new InvalidInputException("Could not find the nodes have defs. Please check the defs in the uses box");
							if(n != null)
								var.addDef(n);
						}					
					}
			...
			while((str = br.readLine()) != null)
			{
				//comments modified by Nan Li
				if(!Pattern.matches(defusePat, str.trim()))
					throw new InvalidInputException("Could not understand the format of the uses for '" + str + "'. " +
							"Please check the syntax: a variable name followed by a space-separated sequence of nodes or edges where the varialbes are defined. ");
				int count = 0;
				st = new StringTokenizer (str, " ");
				Variable var = null;
				while(st.hasMoreTokens())
				{
					...
					else
					{
						...
						if(usesString.indexOf(",") != -1){
							...
							Edge e = dfg.findEdge(srcString, destString);
							if(e == null)
								throw new InvalidInputException("Could not find the edges have uses. Please check the uses in the uses box");
							if(e != null)
								var.addUseOnEdges(e);
						}
						else{
							...
							if(n == null)
								throw new InvalidInputException("Could not find the nodes have uses. Please check the uses in the uses box");
							if(n != null)
								var.addUse(n);
						}
