...
import com.drgarbage.asm.render.impl.Opcodes;
                                                                        ...
                                                                        BytecodeVisualizerPreferenceConstats
{
                                                                                                ...
                                                                                                IMethodSection
    {
        ...
        protected class Block
        {
            ...
            public void appendEnd()
            {
                ...
                endLine = AbstractClassFile.this.lineCount;
            ...
            public void appendStart()
            {
                ...
                startLine = AbstractClassFile.this.lineCount;
        ...
        protected class TryBlock extends Block implements ITryBlock
        {
            ...
            public void computeHandlersEnds(Map<Integer, AbstractInstruction> offsetInstructionMap)
            {
                ...
                if (finallyBlock != null)
                {
                    ...
                    {
                        /*
                         * the above did not function we have to find some better way
                         */
            ...
            public TryBlock createEnclosedTryBlock(ExceptionTableEntry en)
            {
                ...
                {
                    /*
                     * this covers the same area and the handler to add is either not finally or
                     * finallyBlock is not set yet only add handlers
                     */
                    result = this;
                }
                else
        ...
        private ClassFile.ListMap<Integer, Block> blockEnds   = new ClassFile.ListMap<Integer, Block>();
        private ClassFile.ListMap<Integer, Block> blockStarts = new ClassFile.ListMap<Integer, Block>();
         ...
         * 
         * @see
         * com.drgarbage.asm.render.intf.IMethodSection#isLocalVariableTableAvailable()
         */
        ...
        public int getSourceCodeLine(int byteCodeDocLine)
        {
            ...
            if (lineNumberTable == null || lineNumberTable.length == 0 || byteCodeDocLine < firstLine
            ...
            {
                /*
                 * if we have not found it yet we are after the last instruction let us use the
        ...
        private int lookUpLineNumberTable(int offset)
        {
            if (offset != ByteCodeConstants.INVALID_OFFSET)
            {
                ...
                /*
                 * Added by Sergej on 08.05.2008 FIX: if the lineNumberTable includes only one
                 * entry, then this entry is valid for all offsets.
                 */
                ...
                /*
                 * Added by Sergej on 09.05.2008 FIX: Check if the current offset > the startPC
                 * of the last entry. The last entry is not checked in the loop.
                 */
        ...
        private void renderLineNumberTable(LineNumberTableEntry[] lineNumberTable)
        {
            ...
            else
            {
                ...
                tbl.beginRow();
                tbl.appendValue(ByteCodeConstants.LINE_NUMBER_TABLE, ByteCodeConstants.Align.CENTER,
                        tbl.computeWidth());
                tbl.endRow();
                tbl.render();
        ...
        private void renderLocalVariableTable()
        {
            ...
            else
            {
                ...
                tbl.beginRow();
                tbl.appendValue(ByteCodeConstants.LOCAL_VARIABLE_TABLE, ByteCodeConstants.Align.CENTER,
                        tbl.computeWidth());
                tbl.endRow();
        ...
        private void renderExceptionTable()
        {
            ...
            else
            {
                ...
                TextTable tbl = new TextTable(new ByteCodeConstants.Align[]
                { ByteCodeConstants.Align.RIGHT, ByteCodeConstants.Align.RIGHT, ByteCodeConstants.Align.RIGHT,
                        ByteCodeConstants.Align.RIGHT });

                /*
                 * u2 exception_table_length; { u2 start_pc; u2 end_pc; u2 handler_pc; u2
                 * catch_type; } exception_table[exception_table_length];
                 */
                tbl.addRow(new String[]
                { ByteCodeConstants.START_PC, ByteCodeConstants.END_PC, ByteCodeConstants.HANDLER_PC,
                ...
                for (int i = 0; i < exceptionTable.length; i++)
                {
                    ...
                    tbl.addRow(new String[]
                    { String.valueOf(en.getStartPc()), String.valueOf(en.getEndPc()), String.valueOf(en.getHandlerPc()),
                            catch_type_class });
                }
        ...
        private void renderSignature()
        {
            ...
            else
            {
                ...
                try
                {
                    ...
                    {
                        /* constructor */
                        BytecodeUtils.appendMethodDescriptor(classSimpleName, true, (access & ACC_STATIC) != 0,
                                descriptor, 0, localVariableTable, constantPool, sb);
                    }
                    ...
                    else
                    {
                        BytecodeUtils.appendMethodDescriptor(name, false, (access & ACC_STATIC) != 0, descriptor, 0,
                                localVariableTable, constantPool, sb);
                    }
                }
                catch (IOException e)
                {
        ...
        public boolean visitCode(byte[] bytes, int offset, int length)
        {
            ...
            if (exceptionTable != null && exceptionTable.length > 0)
            {
                ...
                Arrays.sort(sortedExceptionTable, new ClassFile.ExceptionTableEntryComparator());
        ...
        public void visitEnd()
        {
            ...
            {
                /*
                 * signature of an abstract or native method we must call it here as
                 * visitCode(...) will not be called for abstract methods
                 */
                renderSignature();
            }
            else
        ...
        {
            // LocalVariableTableParser p = new LocalVariableTableParser(bytes, offset,
            // entryCount);
    ...
    {
        /**
         * The {@link AnnotationVisitor} to which this visitor delegates calls. May be
         * <tt>null</tt>.
         */
    ...
    protected class FieldRenderer extends FieldVisitor implements IFieldSection
    {
         ...
         * 
         * @see
         * com.drgarbage.classfile.render.intf.IOutlineElement#getBytecodeDocumentLine()
         */
    ...
    protected class InstructionRenderer implements IInstructionLine
    {
                ...
                if (instruction instanceof ILocalVariableIndexProvider)
                {
                    if (methodRenderer != null)
                    {
                        ...
                        String name = methodRenderer.localVariableTable.findArgName(
                                ...
                                instruction.getOffset(), methodRenderer.isConstructor(), methodRenderer.isAbstract());
                    ...
                    if (methodRenderer != null && constantPool != null)
                    {
                        ...
                        else if (cpInfo instanceof ConstantFieldrefInfo)
                        {
                            ...
                            ConstantNameAndTypeInfo constantNameAndTypeInfo = (ConstantNameAndTypeInfo) constantPool[constantFieldrefInfo
                                    .getNameAndTypeIndex()];
                            String fieldName = ((ConstantUtf8Info) constantPool[constantNameAndTypeInfo.getNameIndex()])
                                    .getString();
                        ...
                        else if (cpInfo instanceof ConstantMethodrefInfo)
                        {
                            ...
                            else if (instruction.getOpcode() == Opcodes.INVOKESTATIC)
                            {
                                ...
                                String name = ((ConstantUtf8Info) constantPool[constantClassInfo.getNameIndex()])
                                        .getString();
                                methodName = name.replace(ByteCodeConstants.CLASS_NAME_SLASH, JavaLexicalConstants.DOT)
                                        + JavaLexicalConstants.DOT + methodName;
                            }
                            ...
                            BytecodeUtils.appendMethodDescriptor(methodName, isConstructor, isStatic, descriptor, 0,
                                    methodRenderer.localVariableTable, constantPool, sb);
                            // ClassFileDocument.appendMethodDescriptor(methodName, isConstructor, isStatic,
                        ...
                        else if (cpInfo instanceof ConstantClassInfo)
                        {
                            ...
                            {
                                String className = ((ConstantUtf8Info) constantPool[constantClassInfo.getNameIndex()])
                                        .getString();
                                StringBuilder sb = new StringBuilder();
                                sb.append(JavaKeywords.NEW);
                                sb.append(JavaLexicalConstants.SPACE);
                        ...
                        else if (cpInfo instanceof ConstantInvokeDynamicInfo)
                        {
                            ...
                            ConstantNameAndTypeInfo nameAndTypeInfo = (ConstantNameAndTypeInfo) constantPool[constantInvokeDynamicInfo
                                    .getNameAndTypeIndex()];
                            String nameAndType_name = ((ConstantUtf8Info) constantPool[nameAndTypeInfo.getNameIndex()])
                                    .getString();
                            ...
                            boolean isStatic = false;
                            BytecodeUtils.appendMethodDescriptor(nameAndType_name, isConstructor, isStatic,
                                    nameAndType_descr, 0, methodRenderer.localVariableTable, constantPool, sb);
                            const_ = sb.toString();
                        }
                        else
                        {
        ...
        public void render(AbstractMethodRenderer methodRenderer) throws IOException
        {
            ...
            line = AbstractClassFile.this.lineCount;

            /*
             * leading spaces to padd the offsets commented out as this padding does not
             * look nice in combination with try and catch blocks for (int i =
             * String.valueOf(instruction.getOffset()).length(); i <
    ...
    protected class SignatureRenderer extends SignatureVisitor
    {
        ...
        /**
         * Stack used to keep track of array class types. Each element of this stack is
         * a boolean encoded in one bit. The top of the stack is the lowest order bit.
         * Pushing false = *2, pushing true = *2+1, popping = /2.
         */
        ...
        public SignatureVisitor visitTypeArgument(final char tag)
        {
            ...
            {
                declaration.append(JavaLexicalConstants.QUESTION_MARK).append(JavaLexicalConstants.SPACE)
                        .append(JavaKeywords.SUPER).append(JavaLexicalConstants.SPACE);
            }
    ...
    public static final String _EXTENDS_ = new StringBuilder().append(JavaLexicalConstants.SPACE)
            .append(JavaKeywords.EXTENDS).append(JavaLexicalConstants.SPACE).toString();

    public static final String      _IMPLEMENTS_                     = new StringBuilder()
            .append(JavaLexicalConstants.SPACE).append(JavaKeywords.IMPLEMENTS).append(JavaLexicalConstants.SPACE)
            .toString();
    protected static final String   COMMA_                           = new StringBuilder()
            .append(JavaLexicalConstants.COMMA).append(JavaLexicalConstants.SPACE).toString();
    ...
    public static final String      L_JAVA_LANG_DEPRECATED_SEMICOLON = new StringBuilder()
            .append(ByteCodeConstants.L_REFERENCE).append(ByteCodeConstants.JAVA_LANG_DEPRECATED)
            .append(JavaLexicalConstants.SEMICOLON).toString();
    ...
    public AbstractClassFile()
    {
        ...
        if (true)
        {
            ...
            showLineNumberTable = true;
            showSourceLineNumbers = true;
            showLocalVariableTable = true;
            showExceptionTable = true;
            showMaxs = true;
            renderTryCatchFinallyBlocks = true;
     ...
     * (non-Javadoc)
     * 
     * @see com.drgarbage.asm.render.intf.IClassFileDocument#addFieldSection(com.
    ...
    /**
     * Appends a string representation of the given access modifiers to {@link #buf
     * buf}.
     * 
     * @param access
    ...
    /**
     * Append an indentation string if needed and {@link JavaLexicalConstants#DOT}.
     * 
     */
    ...
    /**
     * Append an indentation string if needed and {@link JavaLexicalConstants#GT}.
     * 
     */
    ...
    protected void appendHeaderComment(String classLoadedFrom, String debugTargetName)
    {
        ...
        // if (bundle == null) {
        // /* this should not happen */
        // throw new RuntimeException(""+ CoreConstants.BYTECODE_VISUALIZER_PLUGIN_ID +"
        // not installed.");
        // }
        //
        ...
        if (true)
        {
            ...
            appendHeaderLine(ByteCodeConstants.Version, "1.1");
    ...
    /**
     * Appends the indentation string {@link #indent}-times if we are just at the
     * beginning of a line.
     */
    ...
    /**
     * Append an indentation string if needed and {@link JavaLexicalConstants#LT}.
     * 
     */
     ...
     * 
     * @see
     * com.drgarbage.asm.render.intf.IClassFileDocument#findFieldSection(java.lang.
     * String)
     */
    ...
    public IMethodSection findMethodSection(int line)
    {
        ...
        {
            if (ms.getFirstLine() != ByteCodeConstants.INVALID_OFFSET && ms.getFirstLine() <= line
                    && ms.getLastLine() != ByteCodeConstants.INVALID_OFFSET && ms.getLastLine() >= line)
            {
                return ms;
            }
        }
     ...
     * 
     * @see
     * com.drgarbage.asm.render.intf.IClassFileDocument#findMethodSection(java.lang.
     ...
     * 
     * @see
     * com.drgarbage.classfile.render.intf.IClassFileDocument#isLineInMethod(int)
     */
    public boolean isLineInMethod(int line)
    {
        ...
        else
        {
            ...
            /*
             * the theory here is very simple: test, if the found = (-(<i>insertion
             * point</i>) - 1) is even or odd even means between start and end of a method
             * so return true or false otherwise
             */
            return found % 2 == 0;
        }
    }
    ...
    /**
     * Sets the boolean flag telling if the the <code>try</code>, <code>catch</code>
     * and <code>finally</code> blocks will be rendered (<code>true</code>) by this
